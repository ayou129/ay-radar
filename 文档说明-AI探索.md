...
- `raw_data` 尽量包含有信息增量的评论（如果有），JSON 存入供分析 Agent 使用
...
> **为什么 Agent 不用 Playwright？** 同一 Claude Code 会话内的 sub-agent 共享同一个 MCP Playwright 实例（只有一个 page 引用），与主会话的浏览器操作会互踩。分析所需的信息已在入库时存入 raw_data，无需再打开页面。
