...
2. **产品复刻**：发现热门工具/产品时，评估是否适合快速复刻上线到 [onemake.dev](https://onemake.dev)（一人开发者产品集合，当前产品：KeyHint — macOS 快捷键提示工具）。**不限于 macOS，iOS App 同样在评估范围内**
...
4. 查 `ai_discovery` 中有 `user_note` 的记录，了解用户的偏好反馈：
   - 负面备注（如"这类不需要"）→ 后续减少采集同类内容
   - 正面备注（如"重点关注""想到了 xxx idea"）→ 后续优先探索相关方向
   - 将用户备注作为本轮探索的方向参考，而非硬性过滤规则
...
搜索关键词规范：
- **控制在 2-3 个词**，如 `AI tool`、`macOS app`、`indie hacker`、`Claude Code`、`vibe coding`，长组合词在多数平台返回空结果
- **每组关键词跨所有平台搜索**（X.com、Reddit、知乎、GitHub），不能只在单一平台使用
- X.com 搜索时留意右侧「推荐关注」，是发现同领域新博主的高效入口
...
- **工具复刻评估**（当内容涉及具体工具/产品时）：
  ...
  - 技术上怎么实现？需要什么技术栈？
  - 变现模式？竞品密度如何？
  - 是否适合一人开发者做？和 onemake.dev 现有产品线是否互补？适合 macOS 还是 iOS 还是双端？
...
- [不再采集]zhihu.com（知乎，内容以二手观点文章为主，信息密度低，缺乏一手工具发现）
...
| user_note | TEXT | 用户审批备注——对内容的主观评价、关联 idea、方向偏好等，巡检时作为探索方向参考 |
...
- [2026-03-20] 知乎搜索结果页面经常加载失败（搜索结果区域为空），仅能看到侧边栏热搜。建议：1) 改为直接访问知乎热榜页面提取 AI 相关话题；2) 或使用知乎问题页面直接 URL 而非搜索。（本轮知乎搜索两次均未能加载内容）
