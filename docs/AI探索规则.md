...
<!-- 在此列出你的产品信息，巡检时会参考这些信息评估产品机会 -->
<!-- 示例：
- **YourApp**（yoursite.com）：一句话产品描述
  - 技术栈：xxx
  - 定位：xxx
  - 关注方向：功能拓展、变现路径、同类产品动态
-->
...
<!-- 可添加更多 L2 方向，格式：- **L2 方向名**：描述，同等质量下关注 -->
- **L3 开放**：非 AI 但有价值的内容（indie dev 实战、工具生态、创业方法论等），允许少量采集
...
- 以下维度**按需展开**，不是每条都要覆盖全部：
  ...
  - **工具复刻评估**（当内容涉及具体工具/产品时）：核心功能、技术栈、变现模式、竞品密度、是否适合一人开发者、与自有产品是否互补
